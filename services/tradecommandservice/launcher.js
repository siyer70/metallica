...
const {tradeCommandService} = require('./tradecommandservice');
const Eureka = require('eureka-js-client').Eureka;

function launch(registerPeriodically) {
  const server = http.createServer(tradeCommandService);

  //In Production, no need to set this env variable
  const LISTENPORT = process.env.TRADE_COMMAND_SERVICE_PORT;
  ...
  server.on('listening', function() {
    console.log(`Trade Command Service started on port ${server.address().port}`);
    let tsProtocol = process.env.TRADE_COMMAND_SERVICE_PROTOCOL || 'http';
    let tsHostName = process.env.TRADE_COMMAND_SERVICE_HOSTNAME || 'localhost';
    let tsIpAddr = process.env.TRADE_COMMAND_SERVICE_IPADDRESS || '127.0.0.1';
    let tsPort = `${server.address().port}`;
    let tsApp = process.env.TRADE_COMMAND_SERVICE_APP_REGISTRY_NAME || 'tradecommandservice';
    let tsVipAddress = process.env.TRADE_COMMAND_SERVICE_APP_REGISTRY_NAME || tsApp;
    let tsHostPort = `${tsHostName}:${tsPort}`;
    let tsBaseUrl = tsProtocol + "://" + tsHostPort;
    let tsDCName = process.env.TRADE_COMMAND_SERVICE_DATACENTER_NAME || 'MyOwn';
    let tsDCClass = process.env.TRADE_COMMAND_SERVICE_DATACENTER_CLASS ||
    ...
    // const instances = client.getInstancesByAppId('tradeCommandService');
    ...
    const announce = () => {
      ...
      console.log('Registering trade command service in service locator running in :', BASE_URL);
      request.put(`${BASE_URL}/services/tradecommandservice/${server.address().port}`,
      (err, res) => {
